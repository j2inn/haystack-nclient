...
import { finCsrfFetch, clearFinCsrfTokens } from '../../src/client/finCsrfFetch'
import {
	FIN_AUTH_PATH,
	SKYARC_ATTEST_KEY,
	FIN_AUTH_KEY,
...
describe('finCsrfFetch', function (): void {
	...
	it('does not request CSRF token if already present', async function (): Promise<void> {
		const options: { headers: { [prop: string]: string } } = { headers: {} }
	...
	it('fetches read with no credentials included if already specified', async function (): Promise<void> {
		...
		expect(fetchMock.lastCall(READ)?.[1]?.credentials).toBe('same-origin')
	})
